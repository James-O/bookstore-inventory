    ...
    <header className='w-full bg-transparent fixed top-0 left-0 transition-all ease-in duration-300'>
        <nav className={`py-4 lg:px-24 p-4 ${isSticky? "sticky top-0 left-0 right-0 bg-blue-500":""}`}>
            <div className='flex justify-between text-center text-base gap-8'>
                ...
                <div className='space-x-20 hidden lg:flex items-center'>
                    ...
                    </button> 
                    {
                        ...
                        <div className='flex items-center gap-2'>
                            <Link to='/profile' className=' text-blue-800 3xl'>{user.email}</Link>
                            <Link to={'/logout'} className=' text-blue-800 3xl'>Logout</Link>
                        </div>
                        :
                        <Link to='/login' className=' text-blue-800 3xl'>Login</Link>
                    }
                </div>
                ...
                {
                    navItems.map((item, index)=>(
                        <Link key={index} to={item.path} className="block text-sm text-white uppercase cursor-pointer hover:text-blue-700" onClick={toggleMenu}>{item.link}</Link>
                    ))
                }
                </div>
            </div>
